...
# ======================= DEPENDENCIES ================
source("~/Probe_Finder/code_dir/input_parameters.R")
library(stringr)
library(tidyr)
library(dplyr)
library(readr)
...
gene_seq <- read_lines(fasta_file, skip=1) %>% paste(collapse='')
...
candidates <- candidates %>% subset(num_snps<=max_mismatches)
